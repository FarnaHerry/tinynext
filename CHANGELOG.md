...
> 视觉大版本：岛屿布局单卡化 + 毛玻璃 + 极客黑白色调（对齐 heibu 主题实现）。

### UI：整页单岛布局
- **整页一张岛卡**：下载页 / 设置页从「侧边栏岛卡 + 内容岛卡」两张浮岛合并为一张
  整页岛卡，二级侧边栏与内容区之间用一条**竖向分隔线**（顶到底）区分，不再各自成卡。
  翻页控件仍是内容区里的小浮控件。

### UI：毛玻璃（backdrop blur）
- **弹窗 / 下拉弹层真磨砂**：添加下载 / 删除确认 / 镜像源 / 关于 四个弹窗与排序 / 分页 /
  语言等下拉弹层用 eui 原生 backdrop blur（`.blur(n)` + 半透明填充，官方 demo 同款）。
- **岛卡 / 任务卡 / 翻页半透明玻璃**：整页岛卡、任务卡、翻页改半透明玻璃色（无 blur）——
  全屏 backdrop blur 每帧几十次纹理采样，实测拖慢主题切换，去掉后流畅。

### UI：主题性能
- **修复主题切换卡死**：移除全屏岛卡 blur + 任务卡/翻页的冗余 blur（它们糊的本来就是
  已糊过的岛卡）。现在深色 ↔ 浅色来回切换流畅。

### UI：极客黑白色调（对齐 heibu）
- **纯黑 / 纯白单色**：深色近纯黑底 `(0.04)` + 纯白主色 `(1.0)`、浅色近白底 `(0.96)` +
  纯黑主色 `(0.0)`；文本全部黑 / 白 / 灰，完成 / 失败状态保留柔和绿 / 红作功能区分。
- **选中态切主题色**：tab（直链 / 种子）、主题选择、镜像开关、设置页各开关等可选中
  按钮，选中时切到主色（白 / 黑），文字 / 图标**反色保证可读**（`onPrimaryColor` 对齐
...
- `addMirror` / `removeMirror` 改回调式（`onDone` 在后台线程调用），`DownloadEngine`
  接口同步调整；镜像添加成功清空输入经原子 pending 标志由 UI 线程消费。
